...
# osint-suite Configuration Helper
# Sets up osint-suite.cfg and API key files
# Usage: ./configure-osint.sh
...
echo ""
echo -e "${GREEN}─────── OSINT Config Helper ───────${NC}"
echo ""

# Variables
CONFIG_FILE="osint-suite.cfg"
...
validate_github_token() {
    ...
    if [[ "$token" =~ ^ghp_[A-Za-z0-9]{36}$ ]]; then
        ...
        echo -e "${YELLOW}GitHub tokens must start with 'ghp_' and be 40 characters long.${RESET}"
        return 1
    fi
}
...
validate_gitlab_token() {
    ...
    if [[ "$token" =~ ^glpat-[A-Za-z0-9_.-]{20,60}$ ]]; then
        ...
        echo -e "${RED}[ERROR] Invalid token format: $token${RESET}"
        echo -e "${YELLOW}GitLab tokens must start with 'glpat-'.${RESET}"
        return 1
    fi
}
...
{
    echo "# GitHub Tokens for osint-suite"
    echo "$GITHUB_TOKEN"
} > "$GITHUB_TOKENS_FILE"
